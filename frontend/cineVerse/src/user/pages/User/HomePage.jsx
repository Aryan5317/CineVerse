...
import theatreMovieDetails from "../../services/User/theatreMovieDetails.js"
...
function HomePage() {
    ...
    const [theatreMovieData, setTheatreMovieData] = useState([])
    ...
    useEffect(() => {
        const movieData = async () => {
            try {
                const theatreMovieResponse = await theatreMovieDetails()
                if (theatreMovieResponse) {
                    console.log("Theatre Movie Details are: ", theatreMovieResponse.data)
                    setTheatreMovieData(theatreMovieResponse.data)
                }
            } catch (error) {
                console.log("Error from backend is: ", error)
                setTheatreMovieData({})
            ...
            <div className="min-h-screen bg-[#F8FAFC] pb-24">
                ...
                <div className="mt-8">
                    ...
                    <div className="flex items-center justify-between mb-5">
                        ...
                        <button className="text-sm font-semibold text-blue-600 transition-colors duration-200 hover:text-blue-700">
                            View All →
                        </button>
                    ...
                    <div className="rounded-3xl border border-slate-200 bg-slate-50 p-4">
                        ...
                        <div className="flex gap-5 overflow-x-auto scrollbar-hide snap-x snap-mandatory scroll-smooth pb-2">

                            {theatreMovieData.map((movie, index) => (

                                <button
                                onClick={() => MovieCompleteDetailsPage(movie._id)}
                                    key={index}
                                    className="w-44 flex-shrink-0 overflow-hidden rounded-2xl border border-slate-200 bg-white shadow-sm transition-all duration-300 hover:-translate-y-1 hover:shadow-lg snap-start"
                                >
